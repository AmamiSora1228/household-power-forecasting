...
\noindent\textbf{代码仓库：}\url{https://github.com/AmamiSora1228/household-power-forecasting}
...
本项目使用的数据集来自 UCI ``Individual household electric power consumption'' 数据集~\cite{uci}，记录的是法国索镇（Sceaux）一户家庭 2006 年 12 月至 2010 年 11 月的用电情况，原始粒度为每分钟。数据包含全局有功功率、无功功率、电压、电流强度以及厨房、洗衣房、气候控制三个子表能耗。此外，我们融合了法国气象站的月尺度降水、雨天数、雾天数等信息作为外部变量~\cite{weather}。
...
\section{模型}
\subsection{LSTM 模型}
长短期记忆网络（LSTM）通过门控机制有效捕捉时间序列的长期依赖~\cite{lstm}。我们的 LSTM 编码器将输入序列映射为最后一个时刻的隐状态，再经过两层全连接网络输出未来 $H$ 天的预测值。

\subsection{Transformer 模型}
Transformer 编码器利用自注意力机制直接建模任意时间步之间的关系~\cite{transformer}。输入首先通过线性投影升至 $d_{\text{model}}$ 维，并叠加正弦位置编码；随后经过多层 Transformer 编码器，最后对序列做平均池化并输入全连接解码器。
...
\subsection{代码与复现}
实验代码已整理为模块结构：核心模型与工具位于 \texttt{src/}，可执行脚本（数据预处理、训练、绘图、生成报告）位于 \texttt{scripts/}。运行 \texttt{bash scripts/run\_all.sh} 可一键复现全部结果，或依次执行 \texttt{python3 scripts/data\_preprocess.py}、\texttt{python3 scripts/run\_experiments.py}、\texttt{python3 scripts/plot\_results.py} 与 \texttt{python3 scripts/generate\_report.py}。预处理后的训练/测试数据、实验指标及图表保存于 \texttt{results/} 目录，Overleaf 源文件位于 \texttt{overleaf/} 目录。

\subsection{不足与展望}
\begin{itemize}
    \item 数据集中 2010 年仅覆盖到 11 月下旬，长期 365 天测试曲线的终点受数据限制，未来可补充完整年份数据。
    \item 可进一步引入节假日、家庭成员行为等外部变量，或采用概率预测区间量化不确定性。
    \item 针对长期预测，可尝试 Informer~\cite{informer}、Autoformer 等专为长序列设计的 Transformer 变体。
\end{itemize}
